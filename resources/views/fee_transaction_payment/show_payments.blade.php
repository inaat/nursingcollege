                ...
                <div class="col-md-6">
                    <div class="card card-body bg-light">
                        <p>
                            ...
                            <strong>@lang('english.roll_no'): </strong>{{ ucwords($transaction->student->roll_no) }}
                        </p>
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="card card-body bg-light">
                       <p>
                            <strong>@lang('english.challan_no'):
                            </strong>({{ ucwords($transaction->voucher_no) }})<br>
                            <strong>@lang('english.fee_transaction_date'):
                            </strong>{{ @format_date($transaction->transaction_date) }}<br>
                            <strong>@lang('english.payment_status'): </strong>{{ ucwords($transaction->payment_status) }}
                        </p>
                    </div>
                </div>
                </div>
                        ...
                        @forelse ($payments as $payment)
                            ...
                            @can('fee.add_fee_payment')
                              ...
                              @if(!empty($payment->document_path))
                                &nbsp;
                                <a href="{{$payment->document_path}}" class="btn btn-success btn-xs" download="{{$payment->document_name}}"><i class="fa fa-download" data-toggle="tooltip" title="{{__('english.download_document')}}"></i></a>
                                @if(isFileImage($payment->document_name))
                                &nbsp;
                                  <button data-href="{{$payment->document_path}}" class="btn btn-info btn-xs view_uploaded_document" data-toggle="tooltip" title="{{__('english.view_document')}}"><i class="fa fa-picture-o"></i></button>
                                @endif
