    ...
    {       
        if ($(window).scrollTop() >= ($(document).height() - $(window).height() - 500) - dist && !loading && count < maxCount)
        {
            ...
            @foreach($galleries as $row)
                ...
                $("#waterfall").append("<li><a href='{{ url('photos',$row->id) }}'><div class='m-2 upperBigImg1' style='height:" + height + "px'><img style='height:" + height + "px;width: 100%;' src='{{ $row->thumbnail }}'><div class='detailArr'> <img src='{{ asset('assets/school/images/bx-plus-circle.png') }}' alt=''> <span>{{ $row->title }}</span></div></div></a></li>");
