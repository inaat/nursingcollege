            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-md-4">
                        ...
                        {!! Form::select('campus_id', $campuses, null, ['class' => 'form-select select2 exam-campuses', 'required', 'style' => 'width:100%', 'required', 'placeholder' => __('english.please_select')]) !!}
                    </div>
                    ...
                    <div class="clearfix"></div>
                    <div class="col-12 mt-3 exam-classes">
                        <!-- Loaded class-section table will be injected here -->
                    </div>
...
<script type="text/javascript">
    $(document).ready(function() {
        // Load sections when campus changes
        $(document).on('change', '.exam-campuses', function() {
            let doc = $(this);
            __get_campus_class(doc);
        });

        function __get_campus_class(doc) {
            let campus_id = doc.closest(".row").find(".exam-campuses").val();
            $.ajax({
                method: "GET",
                url: "/exam/get_campus_classe_sections",
                dataType: "html",
                data: { campus_id: campus_id },
                success: function (result) {
                    $('.exam-classes').html(result);
                },
            });
        }
