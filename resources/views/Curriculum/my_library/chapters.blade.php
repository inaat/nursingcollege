                ...
                <div class="col-md-9 ">
                    <div class="row m-0">
                        <div class="flex title uppercase"><h3 class="css-1e7ec0x-Subject">SELECT CHAPTER
                            ({{ $subject->classes->title }} > {{ $subject->name }} )</h3></div>
                            <div class=" btn pe-3 bg-primary text-white book-view">Book View</div>
                            <div class="hide bookdiv-view">
                                <iframe src="{{ url('uploads/subjects/' . $subject->subject_book)}}"  frameborder="0" scrolling="no"width="100%" height="500px" >
                                </iframe>
                            </div>

                     @foreach ($chapters as $chap) 
                    <div class="col-lg-3 col-md-3 col-xs-6 ">
                         <a class="w-24 h-24 xl:w-32 xl:h-32 flex items-center justify-center " href="{{action('Curriculum\MyLibraryController@getChaptersLessons',[$chap->id]) }}">
                                    <img class="css-pxgg1r box d-flex align-items-center justify-content-center"  style="background: #EBA9A5; margin:0 auto;" src="{{ url('uploads/subjects/' . $chap->chapter_icon)}}" alt=" ">
