...
<div class="row" style="@if(!empty($for_pdf))  margin:20px; @endif">
    ...
    <div class="col-md-4 col-sm-4 col-xs-4 @if(!empty($for_pdf)) width-20 f-left @endif">
        ...
        <p><strong>{{ ucwords($student->first_name . ' '. $student->last_name) }}</strong><br>@lang('english.current_address'):{{ $student->std_current_address}}<br> @lang('english.mobile_no'):{{ $student->mobile_no }}
        </p>
    </div>
    <div class="col-md-8 col-sm-7   ">
        <h5 class="mb-0 blue-heading text-center p-4">@lang('english.fee_summary')</h5>
        ...
        <table class="table table-condensed  ">
            <tbody>
                ...
                <tr>
                    <td>@lang('english.total') @lang('english.fee')</td>
                    <td class="align-right">@format_currency($ledger_details['total_fee'])</td>
                </tr>
                <tr>
    ...
    <div class="col-md-12 col-sm-12 @if(!empty($for_pdf)) width-100 @endif">
        <p class="text-center" style="text-align: center;"><strong>@lang('english.fee_ledger_table_heading', ['start_date' => $ledger_details['start_date'], 'end_date' => $ledger_details['end_date']])</strong></p>
