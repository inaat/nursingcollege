        ...
        Menu::create('admin-sidebar-menu', function ($menu) {
            ...
            if (auth()->user()->can('general_settings.view') || auth()->user()->can('session.view') || auth()->user()->can('roles.view') || auth()->user()->can('award.view') || auth()->user()->can('class_level.view') || auth()->user()->can('province.view') || auth()->user()->can('district.view') || auth()->user()->can('city.view') || auth()->user()->can('region.view')) {
                $menu->dropdown(
                    ...
                    function ($sub) {
                        ...
                        if (auth()->user()->can('roles.view')) {
                            $sub->url(
                                action('FormFieldsController@index'),
                                __('Admission Form Custom Fields'),
                                ['icon' => 'fa fas fa-briefcase', 'active' => request()->segment(1) == 'form-fields']
                            );
                        }
                        ...
                        // );
                         $sub->url(
                             action('BatchController@index'),
                             __('Batch'),
                             ['icon' => 'bx bx-disc ', 'active' => request()->segment(1) == 'batches']
                         );
                             $sub->url(
                             action('SemesterController@index'),
                             __('Semester'),
                             ['icon' => 'bx bx-disc ', 'active' => request()->segment(1) == 'semesters']
                         );
                ...
                $menu->dropdown(
                    ...
                    function ($sub) {
                        ...
                        if (auth()->user()->can('exam_result.print')) {
                            ...
                            $sub->url(
                                action('Examination\ExamResultController@sessionWiseExamResult'),
                                __('Session Wise Result Card '),
                                ['icon' => 'bx bx-right-arrow-alt ', 'active' => request()->segment(1) == 'session-wise-exam-result']
                            );
                            $sub->url(
            ...
            if (auth()->user()->can('account.access')) {
                $menu->dropdown(
                    ...
                    function ($sub) {
                        ...
                        $sub->url(
                            action('AccountReportsController@threeColumnCashBookReport'),
                            __('3 Column Cash Book'),
                            ['icon' => 'fa fas fa-book', 'active' => request()->segment(1) == 'account' && request()->segment(2) == 'cashbook']
                        );
                        $sub->url(
            ...
            if (auth()->user()->can('notification.lesson_send_to_students') || auth()->user()->can('notification.weekend_and_holiday')) {
                $menu->dropdown(
                    ...
                    function ($sub) {
                        if (auth()->user()->can('notification.template')) {
                            $sub->url(
                                action('NotificationTemplateController@index'),
                                __('Notification Template'),
                                ['icon' => 'bx bx-right-arrow-alt', 'active' => request()->segment(1) == 'sms-email-templates']
                            );
                        }
            ...
            $menu->dropdown(
                ...
                function ($sub) {
                    if (auth()->user()->can('leave_applications_for_employee.view')) {
                        ...
                        if (auth()->user()->can('mobile_slider.view')) {
                            $sub->url(
                                action('AnnouncementController@index'),
                                __('Announcement'),
                                ['icon' => 'bx bx-right-arrow-alt', 'active' => request()->segment(1) == 'announcements']
                            );
                        }
                        if (auth()->user()->can('mobile_slider.view')) {
                            $sub->url(
                                action('Frontend\SchoolWebsiteController@index'),
                                __('Content'),
                                ['icon' => 'bx bx-right-arrow-alt', 'active' => request()->segment(1) == 'web-settings']
                            );
                        }
                        if (auth()->user()->can('mobile_slider.view')) {
                            $sub->url(
                                action('Frontend\FrontFaqController@index'),
                                __('FAQ'),
                                ['icon' => 'bx bx-right-arrow-alt', 'active' => request()->segment(1) == 'front-faqs']
                            );
                        }
    ...
    private function teacherSidebar()
    {
        Menu::create('admin-sidebar-menu', function ($menu) {
         ...
         //   $this->global($menu);
            ...
            $menu->url(action('TeacherLayout\AssessmentController@index'), __('Assessment'), ['icon' => 'bx bx-file', 'active' => request()->segment(1) == 'assessment'])->order(2);
