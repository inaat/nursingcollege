...
public function getMonthsArray($year)
{
    $monthsArray = [];
...
// Now $start and $end represent the desired date range
...
// Update the $start and $end variables with the modified dates
...
//dd($start_date, $end_date);
            ...
            $total_student_attendance=$this->studentUtil->getStudentTotalAttendances($campus_id, 'present', $start, $end);
            $total_student_late=$this->studentUtil->getStudentTotalAttendances($campus_id, 'late', $start, $end);
            $total_student_leave=$this->studentUtil->getStudentTotalAttendances($campus_id, 'leave', $start, $end);
            $total_student_absent_attendance=$this->studentUtil->getStudentTotalAttendances($campus_id, 'absent', $start, $end);
            //Employee
            $total_employee_attendance=$this->employeeUtil->getEmployeeTotalAttendances($campus_id, 'present', $start, $end);
            $total_employee_absent_attendance=$this->employeeUtil->getEmployeeTotalAttendances($campus_id, 'absent', $start, $end);
            $total_employee_late_attendance=$this->employeeUtil->getEmployeeTotalAttendances($campus_id, 'late', $start, $end);
           ...
           $output['beginning_balance']=$this->feeTransactionUtil->getAccountBeginningBalance($start_date,$end_date, $campus_id,null);
            ...
            $output['took_slc_students'] = $took_slc_students;
            $output['students_gender'] = $students_gender;
            $output['total_student_leave'] = $total_student_leave;
            $output['total_student_attendance'] = $total_student_attendance;
            $output['total_student_absent_attendance'] = $total_student_absent_attendance;
            $output['total_student_late'] = $total_student_late;
            $output['total_employee_late_attendance'] = $total_employee_late_attendance;
             //Khan lala 
             //dd($start_date,$end_date);
            $output['total_progress_collection_during_month']=$this->feeTransactionUtil->getTotalFeePaid(\Carbon::parse($start_date)->startOfMonth()->format('Y-m-d'),\Carbon::parse($end_date)->endOfMonth()->format('Y-m-d'), null, $campus_id);
            ...
            $total=$this->feeTransactionUtil->getFeeTotals(null, $end_date, $campus_id);
             ...
             $total_expense=$this->expenseTransactionUtil->getTotalExpense($start, $end, $campus_id) +$this->hrmTransactionUtil->getTotalHrmPaid($start, $end,null, $campus_id);
             $output['total_expense'] = $total_expense;
             $output['total_hrm_paid_amount'] = $this->expenseTransactionUtil->getTotalExpense(\Carbon::parse($start_date)->startOfMonth()->format('Y-m-d'),\Carbon::parse($end_date)->endOfMonth()->format('Y-m-d'), $campus_id)+$this->hrmTransactionUtil->getTotalHrmPaid(\Carbon::parse($start_date)->startOfMonth()->format('Y-m-d'),\Carbon::parse($end_date)->endOfMonth()->format('Y-m-d'),null, $campus_id);
             
             $total_transport=$this->feeTransactionUtil->getTransportTotals(null, $end_date, $campus_id);
             $total_transport_paid_amount=$this->feeTransactionUtil->getTotalTransportPaid($start, $end, null, $campus_id);

             $output['total_transport_paid_amount']=$total_transport_paid_amount;
             $output['total_transport_progressive_amount']=$this->feeTransactionUtil->getTotalTransportPaid(\Carbon::parse($start_date)->startOfMonth()->format('Y-m-d'),\Carbon::parse($end_date)->endOfMonth()->format('Y-m-d'), null, $campus_id);
             $output['total_transport_dues_this_month']=$total_transport['total_fee_due']+$output['total_transport_progressive_amount'];//+$total_transport_paid_amount;
        ...
        //     dd(['opening_balance'=>$opening_balance,'deposit'=>$deposit,'total_expense'=>$total_expense,
        //     'online'=>$online,'feepaid'=>$this->feeTransactionUtil->getTotalFeePaid(null, $end, null, $campus_id),'debit'=>$debit, 
        //     'total_progress_collection_during_month'=>$output['total_progress_collection_during_month'],'total_transport_paid_amount'=>$this->feeTransactionUtil->getTotalTransportPaid(null, $end, null, $campus_id)
        // ]);
