...
class PromotionController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('Examination.promotion.index')->with(compact('campuses', 'batches','semesters','sessions'));
    }
    public function withoutGetStudentList(Request $request)
    {
       ...
       $batch_id=$input['batch_id'];
        ...
        $batches=Batch::forDropdown(1);
        $semesters=Semester::forDropdown(1);
        return view('Examination.promotion.without_get_student_list')->with(compact('campuses','batches',
        'semesters', 'sessions', 'classes', 'sections', 'campus_id', 'class_id', 'class_section_id','batch_id','semester_id', 'students'));
    }
    ...
    public function create()
    {
        ...
        $batches=Batch::forDropdown(1);
        $semesters=Semester::forDropdown(1);
        return view('Examination.promotion.create')->with(compact('campuses','batches','semesters', 'sessions'));
    }
    ...
    public function getStudentList(Request $request)
    {
        ...
        $students = Student::leftjoin('exam_allocations', 'exam_allocations.student_id', '=', 'students.id')
            ...
            ->where('exam_allocations.session_id', $session_id)
            ->where('students.current_class_id', $class_id)
            ->where('students.batch_id', $batch_id)
            ->where('students.semester_id', $semester_id)
        ...
        $batches=Batch::forDropdown(1);
        $semesters=Semester::forDropdown(1);
        return view('Examination.promotion.student_list')->with(compact('campuses','batches','semesters', 'batch_id','semester_id', 'sessions', 'classes', 'sections', 'terms', 'campus_id', 'class_id', 'exam_create_id', 'class_section_id', 'session_id', 'students'));
    }
...
if (!auth()->user()->can('promotion.with_exam')) {
    abort(403, 'Unauthorized action.');
        ...
        foreach ($input['promotion'] as $key => $value) {
            if ($value['promote']=='continue') {
                ...
                $student->semester_id=$input['semester_id'];
    ...
    public function passOutCreate()
    {
        ...
        $batches=Batch::forDropdown(1);
        $semesters=Semester::forDropdown(1);
        return view('Examination.promotion.pass_out_create')->with(compact('campuses','batches','semesters', 'sessions'));
    }
    public function passOutPost(Request $request)
    {
        ...
        $batches=Batch::forDropdown(1);
        $semesters=Semester::forDropdown(1);
        return view('Examination.promotion.pass_out_get_student_list')->with(compact('campuses','batches','semesters', 
        'sessions', 'classes', 'sections', 'campus_id', 'class_id','batch_id','semester_id', 'class_section_id', 'students'));
    }
