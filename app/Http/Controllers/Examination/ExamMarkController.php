...
if ($user->user_type == 'teacher') {
    ...
    private function classPosition($campus_id,$class_id, $session_id,$exam_create_id)
    {
      ...
      /* old code  $top_students = DB::table('exam_allocations')
    ...
    ->where('session_id', $session_id)
    ->where('exam_create_id', $exam_create_id)
    ->orderBy('rank', 'ASC') // no backticks required here as Laravel handles this part
    ->get();
    ...
    {
       /*old code $top_students = DB::table('exam_subject_results')
        ...
        ->orderBy('rank','ASC')
        ->get();*/
        $top_students = DB::table('exam_subject_results')->select('id', 'total_mark', 'total_obtain_mark')
->selectRaw("FIND_IN_SET(total_obtain_mark, ( SELECT GROUP_CONCAT(DISTINCT `total_obtain_mark` ORDER BY `total_obtain_mark` DESC) FROM exam_subject_results WHERE campus_id = ".$campus_id." AND class_id = ".$class_id." AND subject_id = ".$subject_id." AND session_id = ".$session_id." AND exam_create_id = ".$exam_create_id.")) as `rank`")
->where('campus_id', $campus_id)
->where('class_id', $class_id)
->where('session_id', $session_id)
->where('exam_create_id', $exam_create_id)
