...
use App\Utils\HrmTransactionUti;
...
class HrmPayrollController extends Controller
{
    ...
    * @param EmployeeUtil $employeeUtil
     * @return void
     */
    public function __construct(EmployeeUtil  $employeeUtil,HrmTransactionUtil $hrmTransactionUtil)
    {
...
if (auth()->user()->can('hrm_payment.view')) {
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            $input=$request->input();
            DB::beginTransaction();
            $total_paid=$this->hrmTransactionUtil->getTotalPaid($id);
            $payroll = HrmTransaction::with(['allowance','deduction'])->where('type', 'pay_roll')
            ->findOrFail($id);
            $final_total = $this->employeeUtil->num_uf($input['gross_final_total']);
            if($final_total<$total_paid)
            {
                DB::rollBack();
            ...
            $this->hrmTransactionUtil->updatePaymentStatus($payroll->id, $payroll->final_total);
