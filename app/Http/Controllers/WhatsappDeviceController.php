...
class WhatsappDeviceController extends Controller
{
    ...
    {  
         ...
         $log->whatsapp_id = 8;
         $log->save();
         dispatch(new ProcessWhatsapp('lhss','+923428927305', $log->id, [],$this->whatsappApiService))->delay(Carbon::parse($setTimeInDelay)->addSeconds($addSeconds));
         dd(44);
           
        $whatsapp = WhatsappDevice::first();
          $response=$this->whatsappApiService->sendTestMsg($whatsapp->name,'923428927305','55555');
        // $response= $response['data']['message']['extendedTextMessage']['text'];
        dd($response);
         $wpu = WhatsappDevice::first();
         
        $findWhatsappsession = Http::withoutVerifying()->get('http://whatsapp.sfsc.edu.pk/session/find/'.$wpu->name);
       ...
       $wpu->save();
                ...
                return view('whatsapp-setting.index')->with(compact('wpu'));
