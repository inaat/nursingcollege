...
// ];
            ...
            if ($request->type == "app_settings") {
               // $session_year = $settings['session_year'] ?? "";
                //$calender = !empty($session_year) ? SessionYear::find($session_year) : null;
                 ...
                 $data['app_link'] = 'https://play.google.com/store/apps/details';
                ...
                $data['school_name'] ='iTECH School Management System';
    ...
    protected function getExamResultPdf(Request $request) {
        ...
        try {
            ...
            $details=ExamAllocation::with(['student','campuses','session','current_class','current_class_section','exam_create','exam_create.term','grade','subject_result','subject_result.subject_grade','subject_result.subject_name'])
            ->where('student_id',$request->student_id)->findOrFail($request->exam_id);
            $exam_create_id=$request->exam_id;
        ...
        //     ->header('Content-Disposition', 'inline; filename="fee_card.pdf"');
...
public function getSchool(Request $request) {
    try {
       // $session_year = getSettings('session_year');
        ...
        $data[]=[
            'id'=>1,
            'name'=>'Swat Collegiate Boys Campus',
            'url'=>'https://swatcollegiate.edu.pk/api/'
        ];
        $data[]=[
            'id'=>1,
            'name'=>'The Swat Froebel School and College',
            'url'=>'http://sfsc.edu.pk/api/'
        ];
        $data[]=[
            'id'=>2,
            'name'=>'Abasyn International School',
            'url'=>'https://abasynschools.com/api/'
        ];
        $data[]=[
            'id'=>3,
            'name'=>'Antalya Swat',
            'url'=>'https://antalyaswat.com.pk/api/'
        ];
        $data[]=[
            'id'=>4,
            'name'=>'THE LIGHT HOUSE SCHOOL  SYSTEM',
            'url'=>'https://lhss.sfsc.edu.pk/api/'
        ];
        $data[]=[
            'id'=>5,
            'name'=>'Khyber Computer and Vocational College',
            'url'=>'https://explainerkhan.com/api/'
        ];
        $response = array(
            'error' => false,
