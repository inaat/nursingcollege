...
class AccountController extends Controller
{
    ...
    public function show($id)
    {
        ...
        if (request()->ajax()) {
            ...
            ->leftJoin('classes AS cls', 'c.current_class_id', '=', 'cls.id')
            ->leftJoin('hrm_employees AS em', 'hrm_tp.payment_for', '=', 'em.id')
                            ...
                            ->select(['account_transactions.type', 'account_transactions.amount', 'operation_date',
                                ...
                                DB::raw("CONCAT(COALESCE(c.first_name, ''),' ',COALESCE(c.last_name,''),' (',COALESCE(c.roll_no,''),')',' (',COALESCE(cls.title,''),')' ) as payment_for"),
    ...
    public function __getPaymentDetails($row)
    {
        ...
        if (!empty($row->hrm_transaction_payment_id)) {
            ...
            $details .= '</b> ' . $row->payment_for_pay_roll .'<b>' .'  ' . __('english.employee') ;
        }
...
/**
 ...
 ini_set("pcre.backtrack_limit", "500000000");
        ...
        ->leftJoin('students AS c', 'tp.payment_for', '=', 'c.id')
        ->leftJoin('classes AS cls', 'c.current_class_id', '=', 'cls.id')
        ->leftJoin('class_sections', 'c.current_class_section_id', '=', 'class_sections.id')
        ->leftJoin('hrm_employees AS em', 'hrm_tp.payment_for', '=', 'em.id')
                        ...
                        ->select(['account_transactions.type', 'account_transactions.amount', 'operation_date',
                            ...
                            DB::raw("CONCAT(COALESCE(em.first_name, ''),' ',COALESCE(em.last_name,''),' (',COALESCE(em.employeeID,''),')') as payment_for_pay_roll"),
                            DB::raw("CONCAT(COALESCE(u.surname, ''),' ',COALESCE(u.first_name, ''),' ',COALESCE(u.last_name,'')) as added_by"),
                            DB::raw("CONCAT(COALESCE(c.first_name, ''),' ',COALESCE(c.last_name,''),' (',COALESCE(c.roll_no,''),')',' (',COALESCE(cls.title,''),')',' (',COALESCE(class_sections.section_name,''),')' ) as payment_for"),
                            ])
                         ->groupBy('account_transactions.id')
