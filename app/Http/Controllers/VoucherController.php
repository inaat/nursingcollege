...
class VoucherController extends Controller
{
    ...
    public function receiptPrint($id){
        ...
        $payment = FeeTransactionPayment::with(['student','payment_account','sub_payments','sub_payments.fee_transaction'])->findOrFail($id);
        $student_details = $this->studentUtil->getStudentDue($payment->student->id);
        $total_due=$student_details->total_due;
        $data['is_enabled'] = true;
        $data['html_content'] = view('school-printing.feecard.receipt-print', compact('payment','total_due'))->render();
    ...
    public function singlePrint($id)
    {
        ...
        if (request()->has('account_amount')) {
            $account_amount = request()->get('account_amount');
        ...
        if (request()->ajax()) {
            try {
                ...
                $student = [];
                $current_transaction = FeeTransaction::with(['session', 'campus', 'student','student.batch','student.semester', 'fee_lines.feeHead', 'student_class', 'student_class_section'])->find($id);
                ...
                $miscellaneous_deatil=[];
                $admission_fee=0;
                $tuition_fee=0;
                $library_fee=0;
                $activity_fee=0;
                $exam_fee=0;
                foreach($current_transaction->fee_lines as $fee_line) {
                   if($fee_line->fee_head_id===1){
                    $admission_fee=$fee_line->amount;
                   }
                   elseif($fee_line->fee_head_id===2){
                    $tuition_fee=$fee_line->amount;
                   }
                   elseif($fee_line->fee_head_id===6){
                    $library_fee=$fee_line->amount;
                   }
                   elseif($fee_line->fee_head_id===7){
                    $activity_fee=$fee_line->amount;
                   }
                   elseif($fee_line->fee_head_id===8){
                    $exam_fee=$fee_line->amount;
                   }
                   else{
                    $miscellaneous_charges+=$fee_line->amount;
                    $miscellaneous_deatil []= $fee_line->feeHead->description . ': '  .$fee_line->amount;
                   }
                }
                if($current_transaction->type==='opening_balance'){
                    $miscellaneous_charges+=$current_transaction->final_total;
                    $miscellaneous_deatil []='Opening Balance : '  .$current_transaction->final_total;

                }
                if(!empty($account_amount)){
                    $student[] = [
                        'current_transaction' => $current_transaction,
                        'total_due' => $account_amount ,
                        'admission_fee' => 0,
                        'tuition_fee' => $account_amount,
                        'library_fee' => 0,
                        'activity_fee' => 0,
                        'exam_fee' => 0,
                        'miscellaneous_charges' => 0,
                ...
                $student[] = [
                    ...
                    'miscellaneous_detail' => $miscellaneous_deatil,
                    'account' => $account,
    ...
    private function receiptContent($student)
    {
        ...
        $output['html_content'] = view('school-printing.feecard.voucher', compact('student'))->render();
    ...
    public function TPrint(Request $request)
{
    ...
    if ($validator->fails()) {
        ...
        try {
          ...
          $pdf = \WPDF::loadView('ApiView.fee_card', compact('student','system_detail'));
          $pdf->setPaper('a4')->setOption('margin-top', 10)->setOption('margin-left', 5)->setOption('margin-right', 5);
          // Use your Blade template for PDF generation
          ...
          return response()->json(['success' => 0, 'msg' => 'Something went wrong.'], 500);
      }
