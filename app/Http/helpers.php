...
function getTypeDetailAttribute($type) {
...
if (! function_exists('numberToRoman')) {
    ...
    function numToWord($number, $lang = null, $type, $format = 'international')
    {
        if ($format == 'indian') {
    ...
    function frontMenu()
    {
        $about_nav=FrontAboutUs::select('title', 'id', 'slug')->get();
        return $about_nav;
    }
    function frontEventMenu()
    {
