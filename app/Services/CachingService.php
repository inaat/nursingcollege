...
class CachingService {
     ...
     * @return mixed
     */
    public function schoolLevelCaching($key, callable $callback, int $time = 900) {
     ...
     * @return mixed|string
     */
    public function getSchoolSettings( $key = '*' ) {
        $schoolSettings = app(SchoolSettingInterface::class);
//dd($schoolSettings->builder()->get()->pluck('data', 'name'));
        $settings = $this->schoolLevelCaching(config('constants.CACHE.SCHOOL.SETTINGS'), function () use ($schoolSettings) {
            return $schoolSettings->builder()->get()->pluck('data', 'name');
        });
        if (($key[0] != '*')) {
            /* There is a minor possibility of getting a specific key from the $systemSettings
             * So I have not fetched Specific key from DB. Otherwise, Specific key will be fetched here
