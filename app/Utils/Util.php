  ...
  public function __construct(?WhatsappApiService $whatsappApiService = null)
{
    ...
    public function age($dateOfBirth)
    {
        ...
        return \Carbon::createFromDate($age)->diff(Carbon::now())->format('%y')
        ;
    }
 ...
 */
    ...
    {
       $check_sms=WhatsappLog::latest('initiated_time')->where('status',1)->first();
       if(!empty($check_sms))
       {
      if(strlen(trim($data['mobile_number'])) > 10){
           ...
           $date=Carbon::parse($check_sms->initiated_time)->addSeconds($addSeconds);
           $schedule = 1;
        $log = new WhatsappLog();
        ...
        $setTimeInDelay = $date;
        dispatch(new ProcessWhatsapp($data['sms_body'], $data['mobile_number'], $log->id, [],$this->whatsappApiService))->delay(Carbon::parse($setTimeInDelay));
        }
       ...
       if(strlen(trim($data['mobile_number'])) > 10){
        ...
        $log->status = $schedule == 2 ? 2 : 1;
        $log->schedule_status = $schedule;
        $log->initiated_time =  Carbon::now()->addSeconds($addSeconds);
        $log->whatsapp_id = 8;
        $log->save();
       }
...
if ($permitted_campuses != 'all') {
 ...
 $query->whereNotIn('accounts.id', [3,7,6]);
...
if (!empty($campus_id)) {
 ...
 $details[]=[
   ...
   /**
     * General Notifications Tags
     *
     * @return array
     */
    public static function notificationsTags()
    {
        return [
            'tags' => [
                'attendance' => [
                ...
                'student' => [
                    ...
                    '{student_name}',
                    '{student_campus}',
                    '{student_admission_session}',
                    '{student_current_session_id}',
                    '{student_admission_no}',
                    '{student_admission_date}',
                    '{student_old_roll_no}',
                    '{student_admission_class}',
                    '{student_current_class}',
                    '{student_admission_class_section}',
                    '{student_current_class_section}',
                    '{student_first_name}',
                    '{student_last_name}',
                    '{student_gender}',
                    '{student_birth_date}',
