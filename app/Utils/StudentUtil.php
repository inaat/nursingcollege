...
class StudentUtil extends Util
{
    ...
    public function StudentCreate($request, $guardian_link_id = null)
    {
        if ($guardian_link_id == null) {
            $student_input = $request->only(['campus_id', 'adm_session_id', 'admission_no', 'admission_date', 'roll_no', 'adm_class_id', 'adm_class_section_id',
                ...
                'father_cnic_no', 'mother_name', 'mother_phone', 'mother_occupation', 'mother_cnic_no', 'country_id', 'province_id','discount_per',
                'district_id', 'city_id', 'region_id', 'std_current_address', 'std_permanent_address', 'student_tuition_fee', 'is_transport', 'student_transport_fee','discount_type', 'vehicle_id',
                'remark', 'previous_school_name', 'last_grade', 'student_image', 'opening_balance'
            ,'semester_id','batch_id'
            ]);
            ...
            $student_input['discount_per'] = $this->num_uf($student_input['discount_per']);
            ...
            $student->save();
            $this->qualification($request, $student);
            ...
            $student_input = $request->only(['campus_id', 'adm_session_id', 'admission_no', 'admission_date', 'roll_no', 'adm_class_id', 'adm_class_section_id',
                'first_name', 'last_name', 'gender', 'birth_date', 'category_id', 'domicile_id', 'religion', 'mobile_no', 'email', 'cnic_no',
                'blood_group', 'nationality', 'mother_tongue', 'medical_history', 'is_transport', 'student_tuition_fee', 'student_transport_fee', 'vehicle_id','discount_per',
                'remark', 'previous_school_name', 'last_grade', 'student_image','discount_type', 'opening_balance','semester_id','batch_id']);
            if (!empty($student_input['student_image'])) {
                $student_image = $this->uploadFile($request, 'student_image', 'student_image', 'image', $student_input['roll_no']);
                $student_input['student_image'] = $student_image;
            }
    ...
    public function updateStudent($request, $student_id, $guardian_link_id = null)
    {
        ...
        $student_input = $request->only(['campus_id', 'adm_session_id', 'admission_no', 'admission_date', 'roll_no', 'current_class_id', 'current_class_section_id',
            ...
            'blood_group', 'nationality', 'mother_tongue', 'medical_history', 'father_name', 'father_phone', 'father_occupation',
            'father_cnic_no', 'mother_name', 'mother_phone', 'mother_occupation', 'mother_cnic_no', 'country_id', 'province_id','discount_per',
            'district_id', 'city_id', 'region_id', 'std_current_address', 'std_permanent_address', 'is_transport', 'student_tuition_fee', 'student_transport_fee', 'discount_type','vehicle_id',
            'remark', 'previous_school_name', 'last_grade', 'student_image', 'opening_balance', 'old_roll_no','adm_class_id','adm_class_section_id','semester_id','batch_id']);
        if (!empty($student_input['student_image'])) {
            $student_image = $this->uploadFile($request, 'student_image', 'student_image', 'image', $student_input['roll_no']);
            $student_input['student_image'] = $student_image;
        }
    ...
    public function getStudentList($system_settings_id, $class_id, $class_section_id, $status = null,$already_exists=null,$batch_id=null,$semester_id=null )
    {
        ...
        if (!empty($class_section_id)) {
          ...
          if (!empty($semester_id)) {
            $query->where('students.semester_id', $semester_id);
        }
        if (!empty($status)) {
 ...
 public function qualification($request, $student)
    {
              // Get qualifications data from request
    $qualificationsData = $request->input('qualifications', []);
  // dd($qualificationsData);
    foreach ($qualificationsData as $data) {
       ...
       if (!empty($data['id'])) {
            ...
            $qualification = $student->qualifications()->find($data['id']);
            if ($qualification) {
                $qualification->update([
                    ...
                    'biology_marks' => $data['biology_marks'] ?? null,
                    'board' => $data['board'],
                ]);
            }
        } else {
          
            // Create new qualification for new entries (index 0, 1, etc.)
