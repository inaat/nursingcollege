$(document).ready(function () {
    ...
    var students_table = $("#students_table").DataTable({
        ...
        paging: false,
        ajax: {
            url: "/fee-collection-individual/create",
            data: function (d) {
                ...
                if ($("#students_list_filter_father_name").length) {
                    d.father_name = $(
                        "#students_list_filter_father_name"
                    ).val();
                }
                if ($("#students_list_filter_student_name").length) {
                    d.student_name = $(
                        "#students_list_filter_student_name"
                    ).val();
                }
    ...
    $(document).on(
        "change",
        "#students_list_filter_campus_id,#students_list_filter_class_id,#students_list_filter_class_section_id,#students_list_filter_status",
        function () {
    ...
    $(document).on(
        "keyup",
        "#students_list_filter_father_name,#students_list_filter_student_name,#students_list_filter_roll_no",
        function () {
            if ($.trim($("#current-class").val()) == "") {
                students_table.ajax.reload();
                var $focused = $(":focus");
            ...
            $.ajax({
                ...
                success: function (result) {
                    if (result.success) {
                        ...
                        $("#students_list_filter_roll_no").val(
                            result.data.student_details.roll_no
                        );
                        $("#students_list_filter_student_name").val(
                            result.data.student_details.student_name
                        );
                        $("#students_list_filter_father_name").val(
                            result.data.student_details.father_name
                        );
                        $("#current-class").val(
                            result.data.student_details.current_class +
                                " " +
                                result.data.student_details
                                    .current_class_section
                        );
                        __write_number(
                            $("#tuition-fee"),
                            result.data.student_details.student_tuition_fee
                        );
                        __write_number(
                            $("#transport-fee"),
                            result.data.student_details.student_transport_fee
                        );
                        if (result.data.fee_transaction != null) {
                            $("#voucher_no").val(
                                result.data.fee_transaction.voucher_no
                            );
                        }
                        $(".student_image").attr(
                            "src",
                        ...
                        $(".fee-heads").append(result.html_content);
                        ...
                        $("#transport_datetimepicker").datetimepicker({
                            format:
                                moment_date_format + " " + moment_time_format,
                        });
                        $("input.amount").focus();
                        ...
                        $("input#search_student").focus().select();
                    }
                },
            });
        }
    ...
    $(document).on(
        "change",
        "input.head-amount,input.fee-head-check",
        function () {
            var total = 0;
            var table = $(this).closest("table");
            var transaction_discount_amount = __number_uf(
                $("input.transaction_discount_amount").val()
            )
                ? __number_uf($("input.transaction_discount_amount").val())
                : 0;

            table.find("tbody tr").each(function () {
                if ($(this).find("input.fee-head-check").is(":checked")) {
                    var denomination = __number_uf(
                        $(this).find("input.head-amount").val()
                    )
                        ? __number_uf($(this).find("input.head-amount").val())
                        : 0;
    ...
    $("form#student_transaction_due_form").validate();
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result.status == "due") {
                    ...
                    container
                        .find("form#transaction_payment_add_form")
                        .validate();
                    $(".payment_modal")
    ...
    $(document).on(
        "change",
        "#campus_id,#payment_status,#list_filter_date_range,#transaction_type",
        function () {
            fee_transaction_table.ajax.reload();
        }
    );
    $(document).on(
        "keyup",
        "#other_discount_amount,input.other_amount",
        function () {
            var discount_amount = __number_uf(
                $("input.other_discount_amount").val(),
                false
            );
            var amount = __number_uf($("input.other_amount").val(), false);
            var student_due = __number_uf(
                $("input#other_student_due").val(),
                false
            );
            ...
            total_amount =
                student_due -
                (amount +
                    __number_uf($("input.other_discount_amount").val(), false));
            $("span.other_remaining-balance").text(
                __currency_trans_from_en(total_amount, true)
            );
            if (total_amount < 0) {
                __write_number(
                    $("input#other_balance"),
    ...
    $(document).on('change', '.other_payment_types_dropdown', function () {
        var payment_type = $(this).val();
        var to_show = null;
      alert(220);
        $(this)
            .closest('.other_payment_row')
            .find('.other_payment_details_div')
            .each(function () {
    ...
    $(document).on('change', '.other_payment_types_dropdown', function(e) {
        var payment_type = $('#transaction_payment_add_form .other_payment_types_dropdown').val();
        account_dropdown = $('#transaction_payment_add_form #other_account_id');
    ...
    $(document).on(
        "keyup",
        "#transport_discount_amount,input.transport_amount",
        function () {
            var discount_amount = __number_uf(
                $("input.transport_discount_amount").val(),
                false
            );
            var amount = __number_uf($("input.transport_amount").val(), false);
            var student_due = __number_uf(
                $("input#transport_student_due").val(),
                false
            );
            var discount_amount = amount - discount_amount;
            // if(discount_amount<0){
            ...
            total_amount =
                student_due -
                (amount +
                    __number_uf($("input.transport_discount_amount").val(), false));
            $("span.transport_remaining-balance").text(
                __currency_trans_from_en(total_amount, true)
            );
            if (total_amount < 0) {
                __write_number(
                    $("input#transport_balance"),
                    Math.abs(total_amount)
                );
            }
        }
    ...
    $(document).on('change', '.transport_payment_types_dropdown', function () {
        var payment_type = $(this).val();
        var to_show = null;
      alert(220);
        $(this)
            .closest('.transport_payment_row')
            .find('.transport_payment_details_div')
            .each(function () {
                if ($(this).attr('data-type') == payment_type) {
                    to_show = $(this);
                } else {
    ...
    $(document).on('change', '.transport_payment_types_dropdown', function(e) {
        var payment_type = $('#transaction_payment_add_form .transport_payment_types_dropdown').val();
        account_dropdown = $('#transaction_payment_add_form #transport_account_id');
        if (payment_type == 'advance') {
            if (account_dropdown) {
                account_dropdown.prop('disabled', true);
                account_dropdown.closest('.form-group').addClass('d-none');
