$(document).ready(function() {
    ...
    var students_table = $("#students_table").DataTable({
        ...
        ajax: {
            ...
            data: function(d) {
                ...
                if ($("#students_list_filter_batch_id").length) {
                    d.batch_id = $(
                        "#students_list_filter_batch_id"
                    ).val();
                }
                if ($("#students_list_filter_semester_id").length) {
                    d.semester_id = $(
                        "#students_list_filter_semester_id"
                    ).val();
                }
                ...
                if ($("#students_list_filter_gender").length) {
                    d.gender = $("#students_list_filter_gender").val();
                }
        ...
        columns: [{
            ...
            {
                data: "adm_session_name",
                name: "adm_session_name",
    ...
    $(document).on(
        "change",
        "#students_list_filter_campus_id,#students_list_filter_gender,#students_list_filter_class_id,#students_list_filter_class_section_id,#students_list_filter_status,#only_transport,#students_list_filter_vehicle_id,#students_list_filter_semester_id,#students_list_filter_batch_id",
...
$(document).on("keyup", "#student_tuition_fee", function() {
    // Get the class ID
    var class_id = $(".classes").val();

    // Fetch the tuition fee dynamically
    $.ajax({
        method: "GET",
        url: "/classes/get_class_fee",
        dataType: "json",
        ...
        success: function(result) {
            if (result) {
                var tuition_fee = parseFloat(result.tuition_fee) || 0;
                var discounted_fee = parseFloat(__number_uf($("#student_tuition_fee").val())) || 0;
                var discount_type = $("#discount_type").val();

                if(discount_type=='fixed'){
                    var discount_fixed = (tuition_fee - discounted_fee);
                       // Update the discount fixed  field
                $("#student_discount_per").val(discount_fixed .toFixed(2)); // Display with 2 decimal places

                }else{

                // Calculate the discount percentage
                var discount_percentage = ((tuition_fee - discounted_fee) / tuition_fee) * 100;

                // Update the discount percentage field
                $("#student_discount_per").val(discount_percentage.toFixed(2)); // Display with 2 decimal places
                }
            }
        },
        error: function() {
            alert("Failed to fetch tuition fee. Please try again.");
        },
    });
});

    $(document).on("keyup", "#student_discount_per", function() {
           //Add dropdown for sub units if sub unit field is visible
        var class_id = $(".classes").val();
        $.ajax({
            method: "GET",
            ...
            success: function(result) {
                if (result) {
                    var tuition_fee = parseFloat(result.tuition_fee) || 0;
                    var discount = parseFloat(__number_uf($("#student_discount_per").val())) || 0;
                    var discount_type = $("#discount_type").val();

                    if(discount_type=='fixed'){
                        var discount_fixed = (tuition_fee - discount);
                        // Update the discount fixed  field
                       $("#student_tuition_fee").val(discount_fixed .toFixed(2)); // Display with 2 decimal places
                    }else{
                    // Calculate the discounted fee
                    var discounted_fee = tuition_fee - (tuition_fee * (discount / 100));
                    $("#student_tuition_fee").val(discounted_fee );
