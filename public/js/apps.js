    ...
    $.ajax({
        ...
        success: function(result) {
            ...
            if (result.success == 1 && result.receipt.html_content != '') {
                ...
                var printContents = result.receipt.html_content;
...
$(document).on('click', 'a.print-bank-invoice', function(e) {
    e.preventDefault();
    var href = $(this).data('href');

    // Correcting this line:
    var account_id = $('#account_id').val(); // Assuming it's an input/select
    var account_amount = $('#account_amount').val(); // Assuming it's an input/select

    $.ajax({
        method: 'GET',
        url: href,
        data: { account_id: account_id,account_amount:account_amount }, // Optionally send account_id
        dataType: 'json',
        success: function(result) {
            $('.pace-active'); // This line seems to do nothing—consider removing or modifying
        ...
        error: function(xhr) {
            toastr.error("An error occurred while fetching the invoice.");
        }
    });
});
...
$(document).on('shown.bs.modal', '.exam_date_sheet_modal', function(e) {
  ...
  //exam Print.
   $(document).on('click', 'a.exam-print-invoice', function(e) {
    ...
    var batch_id = $('.global-batch').val();
    var semester_id = $('.global-semester').val();
    var session_id = $('.exam-session').val();
    var exam_create_id = $('.exam_create_id').val();
    var mat = $('.mat').val();
    ...
    $.ajax({
        ...
        url: href,
        dataType: 'json',
        data: {batch_id:batch_id,semester_id:semester_id,campus_id: campus_id,class_id: class_id,class_section_id: class_section_id,session_id: session_id,exam_create_id: exam_create_id,mat:mat},
        success: function(result) {
            $('.pace-active')
            if (result.success == 1 && result.receipt.html_content != '') {
                $('#receipt_section').html(result.receipt.html_content);
